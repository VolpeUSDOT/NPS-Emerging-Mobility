/*
  MKR WAN 1310 -> MatrixPortal M4 Bridge (LoRa.h version)

  - Board: Arduino MKR WAN 1310
  - LoRa:  Sandeep Mistry "LoRa" library (simple P2P, NOT LoRaWAN)

  Function:
    * Receives framed messages from:
        - USB Serial (Serial Monitor)
        - LoRa radio
    * Frame format (NMEA-ish):
        $BSTXT,<station_hex>,<payload>*<CS>

      Example:
        $BSTXT,0A,C\\nCOLOR|WHITE\\nTEXT|0|Next Station\\nTEXT|1|Bos\\\\Camb\\n*32

      - Talker+Type: "BSTXT"
      - station_hex: 2-digit hex station ID (e.g., "0A")
      - payload: text to send to MatrixPortal, with escapes:
          \n  -> newline separator between commands
          \\  -> literal '\'

    * Verifies XOR checksum over characters between '$' and '*'.
    * Filters by STATION_ID.
    * Unescapes payload and forwards it over Serial1 to MatrixPortal M4.

  Wiring:
    MKR WAN 1310            MatrixPortal M4
    ------------------------------------------
    TX (pin 14 / Serial1) -> RX (MatrixPortal Serial1 RX)
    RX (pin 13 / Serial1) <- TX (optional, for back-channel)
    GND                 <-> GND

  NOTE:
    - Make sure you have the "LoRa by Sandeep Mistry" library installed.
    - For MKR WAN 1310, that library expects the Murata firmware >= 1.1.6.
*/
...
// ---------- LoRa Parameters for MKR WAN 1310 ----------
  ...
  for (size_t i = 0; i < in.length(); i++) {
    ...
    if (c == '\\' && i + 1 < in.length()) {
      ...
      if (next == 'n') {
        ...
        // Unknown escape, just keep the next char
        out += next;
        i++;
      }
    } else {
...
bool handleNmeaLikeMessage(const String &msgRaw) {
  ...
  csStr = csStr.substring(0, 2);  // first two chars
  ...
  // body should look like: "BSTXT,0A,<payload>"
...
// Optional: echo any MatrixPortal Serial1 output back to USB Serial
...
void setup() {
  ...
  Serial.println(F("MKR WAN 1310 LoRa -> MatrixPortal Bridge"));
  Serial.print(F("Station ID (hex): 0x"));
  Serial.println(STATION_ID, HEX);

  // UART to MatrixPortal on Serial1 (pins 13=RX, 14=TX)
  Serial1.begin(115200);
  Serial.println(F("Serial1 to MatrixPortal at 115200 bps"));

  // LoRa init
  // NOTE: For MKR WAN 1310, the LoRa library knows how to talk to the Murata module.
  if (!LoRa.begin(LORA_FREQUENCY)) {
    Serial.println(F("[ERROR] LoRa init failed. Check firmware/antenna."));
    while (1) {
      delay(1000);
    }
  }
  ...
  Serial.println(F("Ready. Example frame:"));

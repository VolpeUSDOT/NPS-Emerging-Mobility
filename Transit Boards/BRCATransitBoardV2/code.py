...
URL_ARRIVALS = (
    "https://brycecanyonshuttle.com/Services/JSONPRelay.svc/"
    f"GetStopArrivalTimes?apiKey={API_KEY}&stopIds={STOP_ID}&version=2"
)
URL_CAPS = (
    "https://brycecanyonshuttle.com/Services/JSONPRelay.svc/"
    f"GetVehicleCapacities?apiKey={API_KEY}"
)
...
while True:
    try:
        ...
        times = stops[0].get("Times", [])
        times.sort(key=lambda t: t["Seconds"])
        arrivals = times[:2]

        if capacity_cache is None or (now - last_cap_time) > CAP_CACHE_INTERVAL:
            r = network._wifi.requests.get(URL_CAPS)
            if r.status_code != 200:
                raise RuntimeError("Caps HTTP", r.status_code)
            cap_list = r.json()
            r.close()
            # normalize to integer 0..100
            capacity_cache = {str(c.get("VehicleID") or c.get("VehicleId")): int(float(c.get("Percentage", 0)) * 100) for c in (cap_list or [])}
            last_cap_time = now
            gc.collect()
